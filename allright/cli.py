...
这个模块负责把“用户怎么启动 allright”翻译成 runtime 能理解的对象：
...
def _effective_provider(args):
    # Provider 选择优先级：
    # 1. 用户显式传入 --provider
    # 2. 项目 .env / shell 里的 ALLRIGHT_PROVIDER
    # 3. 代码里的默认 provider
    provider = getattr(args, "provider", None) or provider_env(
        "ALLRIGHT_PROVIDER", default=DEFAULT_PROVIDER
    )
...
def _effective_model(args, provider):
    ...
    if provider == "openai":
        model = provider_env("ALLRIGHT_OPENAI_MODEL", ("OPENAI_MODEL",))
        if model:
            return model
        return DEFAULT_OPENAI_MODEL
    if provider == "anthropic":
        model = provider_env("ALLRIGHT_ANTHROPIC_MODEL", ("ANTHROPIC_MODEL",))
        if model:
            return model
        return DEFAULT_ANTHROPIC_MODEL
    if provider == "deepseek":
        model = provider_env("ALLRIGHT_DEEPSEEK_MODEL", ("DEEPSEEK_MODEL",))
        if model:
            return model
...
def _build_model_client(args):
    ...
    if provider == "openai":
        ...
        base_url = getattr(args, "base_url", None) or provider_env("ALLRIGHT_OPENAI_API_BASE", ("OPENAI_API_BASE",), DEFAULT_OPENAI_BASE_URL)
    ...
    if provider == "anthropic":
        model = _effective_model(args, provider)
        base_url = getattr(args, "base_url", None) or provider_env("ALLRIGHT_ANTHROPIC_API_BASE", ("ANTHROPIC_API_BASE",), DEFAULT_ANTHROPIC_BASE_URL)
        api_key = provider_env(
            "ALLRIGHT_ANTHROPIC_API_KEY",
    ...
    if provider == "deepseek":
        model = _effective_model(args, provider)
        base_url = getattr(args, "base_url", None) or provider_env("ALLRIGHT_DEEPSEEK_API_BASE", ("DEEPSEEK_API_BASE",), DEFAULT_DEEPSEEK_BASE_URL)
        ...
        return AnthropicCompatibleModelClient(
            ...
            temperature=args.temperature,
            timeout=getattr(args, "openai_timeout", getattr(args, "ollama_timeout", 300)),
            # Allright currently uses text-encoded tool calls and does not replay
...
def build_agent(args):
    """根据 CLI 参数装配出一个可运行的 Allright 实例。
    ...
    - 输出：一个新的 `Allright`，或一个从旧 session 恢复出来的 `Allright`
    ...
    store = SessionStore(workspace.repo_root + "/.allright/sessions")
    model = _build_model_client(args)
    session_id = args.resume
    if session_id == "latest":
        session_id = store.latest()
    if session_id:
        return Allright.from_session(
            model_client=model,
            workspace=workspace,
            session_store=store,
...
def build_arg_parser():
    ...
    parser.add_argument(
        ...
        default=None,
        help="Model backend to use. Defaults to ALLRIGHT_PROVIDER or deepseek.",
    )
    parser.add_argument(
        "--model",
        default=None,
        help="Model name override. Defaults to qwen3.5:4b for Ollama, ALLRIGHT_OPENAI_MODEL for openai, ALLRIGHT_ANTHROPIC_MODEL for anthropic, and ALLRIGHT_DEEPSEEK_MODEL for deepseek when set.",
    )
